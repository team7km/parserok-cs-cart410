     ...
     <div class="te-wrap te-general cm-te-section {if $selected_section != "te_general"}hidden{/if}" id="te_general">
        <div class="te-inner-wrap">
            ...
            {if $field.type == "selectbox"}
                <div class="te-general-group">
                {if $field.properties}
                    {$selected_value = $current_style.data.$name}
                    {$current_name = $field.properties.values.$selected_value}
                    <div class="ty-control-group">
                        <label for="elm_te_{$name}" class="clearfix"><span class="te-bg-title float-none">{__($field.description)}</span></label>
                        <div class="te-select-box cm-te-selectbox cm-te-value-changer" tabindex="0"><span>{$current_name}</span><i class="icon-d-arrow"></i>
                            <input type="text" class="hidden cm-te-selectbox-storage" name="style[data][{$name}]" value="{$current_style.data.$name}">
                            <ul class="te-select-dropdown">
                                {foreach from=$field.properties.values key="value" item="value_text"}
                                <li data-ca-select-box-value="{$value}" {if $current_style.data.$name == $value}class="active"{/if}>{$value_text}</li>
                                {/foreach}
                            </ul>
                        </div>
                    </div>
