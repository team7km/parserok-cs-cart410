...
<div class="ty-quick-view__wrapper">
    ...
    <div class="ty-product-block" id="product_main_info_{$obj_prefix}">
        <div class="ty-product-block__wrapper clearfix product-main-info">
        {hook name="products:view_main_info"}
            {if $product}
                ...
                <div class="vs-title clearfix">
                    ...
                    {if !$hide_title}
                        <h1 class="ty-product-block-title">
                            <a href="{$product_detail_view_url|fn_url}" class="ty-quick-view__title vs-qv-title" {live_edit name="product:product:{$product.product_id}"}>{$product.product nofilter}</a>
                        </h1>
                    {/if}
                </div>
                ...
                <div class="brand-wrapper clearfix">
                    ...
                    <div class="advanced-options-wrapper ty-float-left">
                    ...
                    <div class="ty-float-right vs-product-social clearfix">
                ...
                <div class="clearfix">
                    <div class="image-wrap ty-float-left">
                        {hook name="products:quick_view_image_wrap"}
                            {if !$no_images}
                                <div class="image-border ty-product-block__img cm-reload-{$obj_prefix}{$obj_id}" id="product_images_{$obj_prefix}{$obj_id}_update">
                                    {assign var="discount_label" value="discount_label_`$obj_prefix``$obj_id`"}
                                    {$smarty.capture.$discount_label nofilter}

                                    {include file="views/products/components/product_images.tpl" product=$product show_detailed_link="Y" image_width=$settings.Thumbnails.product_quick_view_thumbnail_width image_height=$settings.Thumbnails.product_quick_view_thumbnail_height}
                                <!--product_images_{$obj_prefix}{$obj_id}_update--></div>
                            {/if}
                        {/hook}
                    </div>

                    <div class="ty-product-block__left product-info">
                        ...
                        <div class="{if $smarty.capture.$old_price|trim || $smarty.capture.$clean_price|trim || $smarty.capture.$list_discount|trim}prices-container {/if}price-wrap clearfix product-detail-price">
                            ...
                            {if !$smarty.capture.$old_price|trim || 
                                $quick_view}<p class="actual-price">{/if}
                        ...
                        {if $product.promo_text|trim || $smarty.capture.$prod_descr|trim}
                        ...
                        {if $capture_options_vs_qty}{capture name="product_options"}{$smarty.capture.product_options nofilter}{/if}
                            <div class="options-wrapper indented {if !$product.product_options}hidden{/if}">
