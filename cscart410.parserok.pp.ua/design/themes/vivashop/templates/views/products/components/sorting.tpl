...
{/if}
<a class="ty-sort-container__views-a {$ajax_class} {if $layout == $selected_layout}active{/if}" data-ca-target-id="{$pagination_id}" href="{"`$curl`&sort_by=`$search.sort_by`&sort_order=`$sort_order`&layout=`$layout`"|fn_url}" rel="nofollow"><i class="vs-icon-{$layout|replace:"_":"-"}"></i></a>
{/if}
{/foreach}
</div>
{/if}
...
{/if}
{assign var="vs_custom_columns" value=$vs_columns|default:$settings.Appearance.columns_in_products_list}
...
{assign var="product_steps" value=$vs_custom_columns|fn_get_product_pagination_steps:$settings.Appearance.products_per_page}
