...
{if $pagination.total_pages > 1}
    ...
    <div class="ty-pagination vs-pagination">
        ...
        {/if}
        <a data-ca-scroll=".cm-pagination-container" class="ty-pagination__item ty-pagination__btn {if $pagination.prev_page}ty-pagination__prev cm-history {$ajax_class}{/if}" {if $pagination.prev_page}href="{"`$c_url`&page=`$pagination.prev_page`"|fn_url}" data-ca-page="{$pagination.prev_page}" data-ca-target-id="{$id}"{/if}><i class="vs-icon-arrow-left"></i></a>

        {if count($pagination.navi_pages)>8}
            {$half=round(count($pagination.navi_pages) / 2)}
            {if $pagination.current_page<$half}
                {if count($pagination.navi_pages)==10}
                    {$t=array_pop($pagination.navi_pages)}
                {/if}
                {$hide_last=true}
            {else}
                {if count($pagination.navi_pages)==10}
                    {$t=array_shift($pagination.navi_pages)}
                {/if}
                {$hide_first=true}
            {/if}
        {/if}
        <div class="ty-pagination__items">
        {foreach from=$pagination.navi_pages item="pg" name="vs_pag"}
            {if $pg != $pagination.current_page}
                    <a data-ca-scroll=".cm-pagination-container" href="{"`$c_url`&page=`$pg``$extra_url`"|fn_url}" data-ca-page="{$pg}" class="cm-history ty-pagination__item {$ajax_class} {if ($smarty.foreach.vs_pag.last && $hide_last) || ($smarty.foreach.vs_pag.first && $hide_first)}hidden-phone{/if}" data-ca-target-id="{$id}">{$pg}</a>
            {else}
