{if $content|trim}
    <div class="{$sidebox_wrapper|default:"ty-footer"}{if isset($hide_wrapper)} cm-hidden-wrapper{/if}{if $hide_wrapper} hidden{/if}{if $block.user_class} {$block.user_class}{/if}{if $content_alignment == "RIGHT"} ty-float-right{elseif $content_alignment == "LEFT"} ty-float-left{/if}">
        <h2 class="ty-footer-general__header {if $header_class} {$header_class}{/if} cm-combination clearfix" id="sw_footer-general_{$block.block_id}">
        ...
        <div class="ty-footer-general__body clearfix" id="footer-general_{$block.block_id}">{$content|default:"&nbsp;" nofilter}</div>
    </div>
