...
if ($mode == 'async') {
    ...
    while (!empty($q)) {
        ...
        db_query("UPDATE ?:se_queue SET ?u WHERE queue_id = ?i", array(
            'status' => 'processing',
            'started' => TIME
        ), $q['queue_id']);
...
function fn_se_get_product_quantity($product_data)
{
    if (isset($product_data['tracking']) && $product_data['tracking'] == ProductTracking::DO_NOT_TRACK) {
        return 1;
    } else {
        return $product_data['amount'];
    }
}
...
function fn_se_prepare_product_data($product_data, $usergroups, $company_id, $lang_code)
{
    ...
    $entry = array(
        ...
        'quantity' => array(
            'value' => fn_se_get_product_quantity($product_data),
    ...
    if (!empty($product_data['need_age_verification'])) {
        $entry['age_limit'] = array(
            'name' => 'age_limit',
            'title' => __('age_limit'),
            'type' => 'int',
            'value' => $product_data['need_age_verification'] == 'Y' ? $product_data['searchanise_age_limit'] : 0,
        );
    }
