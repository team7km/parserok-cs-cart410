...
class Service
{
    ...
    public function generateProductsAndAttachToGroup($group_id, $base_product_id, array $combination_ids)
    {
        ...
        $combinations = $this->getFeaturesVariantsCombinations($group->getFeatures(), $group->getProductIds());
    ...
    public function generateProductsAndCreateGroup($base_product_id, array $combination_ids, GroupFeatureCollection $group_features = null)
    {
        ...
        $combinations = $this->getFeaturesVariantsCombinations($group_features, []);
     ...
     * @return array
     */
    public function getFeaturesVariantsCombinations(GroupFeatureCollection $group_features, array $exists_product_ids = [])
    {
        $result = [];
        ...
        $features_to_purpose = array_column($features, 'purpose', 'feature_id');
        ...
        foreach ($combinations as $combination) {
            $variant_ids = array_values($combination);
            $key = $this->product_repository->generateCombinationId($variant_ids);
            $name_parts = [];
            $parent_name_parts = [];

            $parent_combination_id = $this->product_repository->generateCombinationId(array_diff_key($combination, array_flip($variation_product_feature_ids)));

            $item = [
                'variation_combination_id' => $key,
            ...
            foreach ($combination as $feature_id => $variant_id) {
                ...
                $variant['feature'] = $feature['description'];
                $variant['feature_purpose'] = $features_to_purpose[$feature_id];

                $name_parts[] = sprintf('%s: %s', $variant['feature'], $variant['variant']);
                $item['variants'][$variant_id] = $variant;

                if (FeaturePurposes::isCreateCatalogItem($features_to_purpose[$feature_id])) {
                    $parent_name_parts[] = sprintf('%s: %s', $variant['feature'], $variant['variant']);
                }
            }

            $item['name'] = implode(', ', $name_parts);
            $item['parent_name'] = implode(', ', $parent_name_parts);
