...
{if $discussion && $discussion.type != "D"}
    <div class="discussion-block" id="{if $container_id}{$container_id}{else}content_discussion{/if}">
        ...
        <div id="posts_list_{$object_id}">
            {if $discussion.posts}
                ...
                {foreach from=$discussion.posts item=post name="vs_post"}
                    <div class="ty-discussion-post__content ty-mb-l {if $smarty.foreach.vs_post.last}last{/if}">
    ...
    {if "CRB"|strpos:$discussion.type !== false && !$discussion.disable_adding}
        <div class="ty-discussion-post__buttons buttons-container">
            {include file="buttons/button.tpl" but_id="opener_new_post" but_text=$new_post_title but_role="submit" but_target_id="new_post_dialog_`$obj_id`" but_meta="cm-dialog-opener cm-dialog-auto-size ty-btn__secondary" but_rel="nofollow"}
        </div>
        {if $object_type != "P"}
