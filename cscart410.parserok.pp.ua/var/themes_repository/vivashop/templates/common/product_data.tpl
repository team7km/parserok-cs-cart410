...
{if $show_add_to_cart}
<div class="cm-reload-{$obj_prefix}{$obj_id} {if $show_vs_icon_buttons}ty-float-left clearfix{/if} {$add_to_cart_class}" id="add_to_cart_update_{$obj_prefix}{$obj_id}">
        ...
        {if $product.has_options && !$show_product_options && !$details_page}
            {if $show_vs_icon_buttons}
                {include file="buttons/button.tpl" 
                    ...
                    but_role="vs_icon"
                    but_name=""
                    vs_icon="vs-icon-select-options"
            ...
            {else}
                ...
                {include file="buttons/button.tpl" 
                    ...
                    but_name=""
                    vs_icon="vs-icon-select-options"
                    but_extra_class="vs-opt"}
            {/if}
        {elseif $show_vs_icon_buttons}
...
{hook name="products:buttons_block"}
    {capture name="vs_buttons_product"}
        {if !($product.zero_price_action == "R" && $product.price == 0) && !($settings.General.inventory_tracking == "Y" && $settings.General.allow_negative_amount != "Y" && (($product_amount <= 0 || $product_amount < $product.min_qty) && $product.tracking != "ProductTracking::DO_NOT_TRACK"|enum && $product.tracking != "ProductTracking::TRACK_WITH_OPTIONS"|enum) && $product.is_edp != "Y")}

            {if $smarty.capture.buttons_product|trim != '&nbsp;'}
                <{if $show_vs_icon_buttons}div class="ty-float-left"{elseif $separate_buttons && !$quick_view}div class="buttons-container clearfix"{elseif $separate_buttons && $quick_view}div class="qv-buttons-container clearfix"{else}span{/if} id="cart_add_block_{$obj_prefix}{$obj_id}">
                    ...
                    {/if}
                </{if $show_vs_icon_buttons}div{elseif $separate_buttons}div{else}span{/if}>
            {/if}
        ...
        {/if}
    {/capture}
    {if !$show_vs_icon_buttons && !$details_page}
        {$smarty.capture.vs_buttons_product nofilter}

        {if $settings.Appearance.enable_quick_view == 'Y' && $vs_list}
            {include file="views/products/components/quick_view_link.tpl" quick_nav_ids=$quick_nav_ids show_vs_icon_buttons=true}
        {/if}
    ...
    {if $show_list_buttons}
        ...
        {if $smarty.capture.$capture_buy_now|trim}
            {if $separate_buttons}<div class="add-buttons-wrap {if $cart_button_exists || (($product.out_of_stock_actions == "S") && ($product.tracking != "ProductTracking::TRACK_WITH_OPTIONS"|enum))} no-margin{/if}"><div class="add-buttons-inner-wrap {if $details_page}clearfix{/if}">{/if}
                <{if $separate_buttons || $vs_separate_buttons}div{else}span{/if} id="cart_buttons_block_{$obj_prefix}{$obj_id}" class="add-buttons add-to-wish clearfix{if $show_vs_icon_buttons} ty-float-left{/if}">
                    {$smarty.capture.$capture_buy_now nofilter}
                </{if $separate_buttons || $vs_separate_buttons}div{else}span{/if}>
...
{capture name="qty_`$obj_id`"}
    {hook name="products:qty"}
        ...
        {if $show_qty && $product.is_edp !== "Y" && $cart_button_exists == true && ($settings.General.allow_anonymous_shopping == "allow_shopping" || $auth.user_id) && $product.avail_since <= $smarty.const.TIME || ($product.avail_since > $smarty.const.TIME && $product.out_of_stock_actions == "B")}
            {if $product.prices && !$vs_qty_discount}
