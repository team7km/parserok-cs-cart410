...
<body>
    {hook name="index:body"}
        ...
        {elseif $runtime.customization_mode.live_editor}
            {include file="common/toolbar.tpl"
                title=__("on_site_live_editing")
                href="customization.disable_mode?type=live_editor&return_url={$config.current_url|urlencode}"
            }
        {elseif $runtime.customization_mode.block_manager && $location_data.is_frontend_editing_allowed}
            {include file="common/toolbar.tpl"
                title=__("on_site_layout_editing", ["[page]" => $location_data.name])
                href="customization.disable_mode?type=block_manager&return_url={$config.current_url|urlencode}"
            }
        {elseif "THEMES_PANEL"|defined}
