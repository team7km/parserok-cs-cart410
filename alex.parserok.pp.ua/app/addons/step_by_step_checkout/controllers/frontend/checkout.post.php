...
if ($mode === 'checkout') {
    ...
    if (!empty($auth['user_id'])) {
        ...
        if (!empty($_REQUEST['profile_id'])) {
            ...
            $user_profile = fn_get_user_info($auth['user_id'], empty($_REQUEST['profile']), $cart['profile_id']);
            list($cart, $auth) = fn_checkout_update_user_data($cart, $auth, $user_profile, true, $auth['user_id']);
