...
if ($mode === 'process') {
    ...
    $options = array_slice($options, ($page - 1) * $items_per_page, $items_per_page, true);
    ...
    $view->assign([
        ...
        'by_combinations'                  => $by_combinations,
        'search'                           => [
            'total_items'    => $total_options_count,
            'page'           => $page,
...
function fn_product_variations_convert_process($by_variations, $by_combinations, $product_ids)
{
    ...
    fn_set_progress('title', __('product_variations.converter.progress.options.title'));
    Registry::set('runtime.product_variations_converter_mode', 'options');

    foreach ($options as &$option) {
        fn_set_progress('echo', $option['option_name'], false);

        $result = fn_product_variations_convert_process_option($option);

        if ($result === false) {
            $errors[] = __('product_variations.converter.progress.options.error', [
        ...
        fn_set_progress('echo', $option['option_name']);
    }
    unset($option);
...
function fn_product_variations_convert_process_product_with_combinations($product_id, $combinations)
{
    ...
    foreach ($combinations as $key => &$combination) {
        ...
        foreach ($prices as $item) {
            $item['price'] = fn_apply_options_modifiers($combination['variation_options'], $item['price'], 'P');
        ...
        $combination['weight'] = fn_apply_options_modifiers($combination['variation_options'], $product_row['weight'], 'W');
...
function fn_product_variations_convert_process_option(array $option)
{
    static $new_features = [];
    static $new_variants = [];
    ...
    if (empty($feature_id)
        && (isset($new_features[$option['option_name']]) || isset($new_features[$option['option_inner_name']]))
    ) {
        $feature_id = isset($new_features[$option['option_name']]) ? $new_features[$option['option_name']] : $new_features[$option['option_inner_name']];
    ...
    if (empty($feature_id)) {
        $feature_id = fn_update_product_feature([
            ...
            'company_id'    => fn_allowed_for('MULTIVENDOR') ? 0 : $option['company_id'],
        ...
        $new_features[$option['option_name']] = $feature_id;
        $option['feature_id'] = $feature_id;
    }

    if (fn_allowed_for('ULTIMATE')) {
        foreach ($option['company_ids'] as $company_id) {
    ...
    foreach ($option['variants'] as &$variant) {
        ...
        if (isset($new_variants[$feature_id][$variant['variant']])) {
            $variant_id = $new_variants[$feature_id][$variant['variant']];
        } else {
            ...
            $new_variants[$feature_id][$variant['variant']] = $variant_id;
        }
...
function fn_product_variations_convert_get_child_products($limit = 1000)
{
    ...
    do {
        $products = db_get_array(
            'SELECT p.product_id, p.__variation_options, p.parent_product_id, pd.product, p.company_id, p.__is_default_variation, ppd.product AS parent_product FROM ?:products AS p'
...
function fn_product_variations_convert_get_products_using_combinations($filter_product_ids, $limit = 1000)
{
    ...
    do {
        ...
        $products = db_get_array(
            'SELECT p.product_id, poi.product_code, poi.amount, poi.combination, poi.position, poi.combination_hash, pd.product, p.company_id FROM ?:products AS p'
            . ' INNER JOIN ?:product_options_inventory AS poi ON poi.product_id = p.product_id'
            . ' LEFT JOIN ?:product_descriptions AS pd ON pd.product_id = p.product_id AND pd.lang_code = ?s ?p'
            . ' LIMIT ?i OFFSET ?i',
            CART_LANGUAGE, $condition, $limit, $offset
        );
...
function fn_product_variations_convert_find_usage_options($by_variations = true, $by_combinations = true, array $filter_product_ids = [])
{
    ...
    if ($option_ids) {
        ...
        $raw_options = db_get_array(
            'SELECT po.option_id, po.position, po.product_id, po.option_type, po.company_id, pod.option_name, pod.internal_option_name, pfd.description, pfd.feature_id FROM ?:product_options AS po'
            . ' LEFT JOIN ?:product_options_descriptions AS pod ON pod.option_id = po.option_id AND pod.lang_code = ?s'
            . ' LEFT JOIN ?:product_features_descriptions AS pfd ON (pfd.description = pod.option_name OR pfd.description = pod.internal_option_name) AND pfd.lang_code = ?s'
            . ' LEFT JOIN ?:product_features AS pf ON pfd.feature_id = pf.feature_id AND pf.feature_type IN (?a)'
            . ' WHERE po.option_id IN (?n)',
            CART_LANGUAGE, CART_LANGUAGE, [ProductFeatures::TEXT_SELECTBOX, ProductFeatures::NUMBER_SELECTBOX], $option_ids
        );

        foreach ($raw_options as $item) {
            $option_id = $item['option_id'];

            $options[$option_id]['feature_id'] = $item['feature_id'];
            $options[$option_id]['feature_name'] = $item['description'];
        ...
        if (fn_allowed_for('ULTIMATE')) {
            $option_company_map = db_get_hash_multi_array(
                'SELECT share_company_id, share_object_id FROM ?:ult_objects_sharing WHERE share_object_type = ?s AND share_object_id IN (?n)',
        ...
        foreach ($options as &$option) {
            if (isset($option_company_map[$option['option_id']])) {
                $option['company_ids'] = array_unique(array_merge(
                    $option['company_ids'],
                    $option_company_map[$option['option_id']]
                ));
            }
            ...
            if (isset($option['option_type']) && $option['option_type'] === ProductOptionTypes::CHECKBOX) {
                ...
                if ($option['feature_id']) {
                    $feature_variants = db_get_hash_single_array(
                        'SELECT pfvd.variant_id, pfvd.variant FROM ?:product_feature_variant_descriptions AS pfvd'
                        . ' LEFT JOIN ?:product_feature_variants AS pfv ON pfv.variant_id = pfvd.variant_id'
                        . ' WHERE pfvd.variant IN (?a) AND pfvd.lang_code = ?s AND pfv.feature_id = ?i',
                        ['variant', 'variant_id'],
                        [__('no'), __('yes')], CART_LANGUAGE, $option['feature_id']
                    );
                }
                ...
                foreach ($raw_variants as &$item) {
                    ...
                    if (isset($feature_variants[$item['variant_name']])) {
                        $item['feature_variant_id'] = $feature_variants[$item['variant_name']];
                    } else {
                        $item['feature_variant_id'] = null;
                    }
                }
                unset($item);
            } else {
                if ($option['feature_id']) {
                    $raw_variants = db_get_array(
                        'SELECT pov.variant_id, povd.variant_name, pfvd.variant_id AS feature_variant_id FROM ?:product_option_variants AS pov'
                        . ' LEFT JOIN ?:product_option_variants_descriptions AS povd ON povd.variant_id = pov.variant_id AND povd.lang_code = ?s'
                        . ' LEFT JOIN ?:product_feature_variant_descriptions AS pfvd ON pfvd.variant = povd.variant_name AND pfvd.lang_code = ?s'
                        . ' LEFT JOIN ?:product_feature_variants AS pfv ON pfv.variant_id = pfvd.variant_id AND pfv.feature_id = ?i'
                        . ' WHERE pov.option_id = ?i AND pov.variant_id IN (?n) ORDER BY pov.position ASC',
                        CART_LANGUAGE, CART_LANGUAGE, $option['feature_id'], $option['option_id'], $variant_ids
                    );
                } else {
                    $raw_variants = db_get_array(
            ...
            foreach ($raw_variants as $variant) {
                $option['variants'][$variant['variant_id']] = [
                    'variant_id'         => $variant['variant_id'],
                    'feature_variant_id' => isset($variant['feature_variant_id']) ? $variant['feature_variant_id'] : null,
