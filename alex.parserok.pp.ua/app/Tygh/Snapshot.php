...
class Snapshot
{
    ...
    protected static function applyConfig($snapshot_curr, $snapshot_dist, $config)
    {
        ...
        $skipped_changed_files = [
            '/app/addons/step_by_step_checkout/layouts/layouts_lite_checkout_multivendor.xml',
            '/app/addons/step_by_step_checkout/layouts/layouts_lite_checkout_ultimate.xml',
        ];

        foreach ($skipped_changed_files as $path_def) {
            $hash_dist = array_search($path_def, $snapshot_dist['files']);
            $hash_curr = array_search($path_def, $snapshot_curr['files']);

            if ($hash_dist === false || $hash_curr === false) {
                continue;
            }

            // default snapshot: remove original hash
            unset($snapshot_dist['files'][$hash_dist]);
            // default snapshot: add current hash
            $snapshot_dist['files'][$hash_curr] = $snapshot_curr['files'][$hash_curr];
        }
