...
class Manager
{
    ...
    public function storeLocation($update_profile = false)
    {
        ...
        $ship_to_another = fn_check_shipping_billing($this->user_data_storage->getAll(), fn_get_profile_fields('C'));
        if (!$ship_to_another) {
        ...
        if ($this->user_id && $update_profile) {
            if ($profile_id = $this->user_data_storage->get('profile_id')) {
                $location['profile_id'] = $profile_id;
            }
            fn_update_user_profile($this->user_id, $location, 'update');
        }
     ...
     */
    public function getLocationField($array, $field, $default_value = null)
    {
        $address_fields_prefixes = ['', SHIPPING_ADDRESS_PREFIX . '_', BILLING_ADDRESS_PREFIX . '_'];
     ...
     *
     * @param array $user_data      User data
     * @param bool  $update_profile Whether to update user's profile
     *
     * @return array
     */
    public function setLocationFromUserData(array $user_data, $update_profile = false)
    {
