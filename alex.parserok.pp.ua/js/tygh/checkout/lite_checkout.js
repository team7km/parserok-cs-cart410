    ...
    var methods = {
        ...
        serializeCheckout: function (userDataExtend) {
            ...
            if (userData.fullname) {
                var fullNameParts = methods.splitFullname(userData.fullname);
                userData.firstname = fullNameParts[0];
                userData.lastname = fullNameParts[1];
            }
            if (userData.s_fullname) {
                var shippingNameParts = methods.splitFullname(userData.s_fullname);
            ...
            if ($captcha) {
                if ($.is.blank(data[$captcha.prop('name')])) {
                    $.ceNotification(
                        ...
                        'litecheckout_captcha_error_' + Math.random().toString().str_replace('.', '_')
                    );
                    return false;
                }
            }
