<tr>
    ...
    {foreach $selected_features as $feature}
        ...
        {else}
            <td><select name="products_variation_feature_values[{$product.product_id}][{$feature.feature_id}]" class="input-hidden product-variations__table-select">
            {foreach $feature.variants as $variant}
                <option
                    value="{$variant.variant_id}"
                    {if $product.variation_features[$feature.feature_id].variant_id == $variant.variant_id}selected{/if}
    ...
    <td width="13%" data-th="{__("price")}">
        <input type="text" name="products_data[{$product.product_id}][price]" value="{$product.price|fn_format_price:$primary_currency:null:false}" class="input-full input-hidden"/>
    </td>
    <td width="9%" data-th="{__("quantity")}">
        {if $is_form_readonly}
            <span>{$product.amount}</span>
        {else}
            <input type="text" name="products_data[{$product.product_id}][amount]" size="6" value="{$product.amount}" class="input-full input-hidden" />
        {/if}
    </td>
