...
function fn_get_customer_location($auth, $cart, $billing = false)
{
    ...
    if (empty($u_info)) {
        ...
        $u_info += [
            ...
            $prefix . 'zipcode' => Registry::get('settings.Checkout.default_zipcode'),
        ];
    }
...
function fn_checkout_update_user_data($cart, $auth, $user_data, $ship_to_another, $user_id)
{
    ...
    if ($user_id) {
        ...
        $send_notification = isset($user_data['email']) && $user_data['email'] !== $email;
...
function fn_checkout_get_user_profiles($auth)
{
    ...
    return [$user_profiles, $selectable_profiles];
}
 ...
 * @return bool
 */
function fn_checkout_set_cart_profile_id(&$cart, $profile_id)
{
