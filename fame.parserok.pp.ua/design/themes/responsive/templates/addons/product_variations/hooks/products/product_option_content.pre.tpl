                    ...
                    <bdi>
                        {if $feature.feature_style === $feature_style_images}
                            {foreach $feature.variants as $variant}
                                ...
                                <p>{$variant.variant}</p>
                            {/foreach}
                        {elseif $feature.feature_style === $feature_style_dropdown}
                        ...
                        {capture name="variant_images"}
                            {foreach $feature.variants as $variant}
                                <a href="{$product_url|fn_link_attach:"product_id={$variant.product.product_id}"|fn_url}"
                                   class="ty-product-options__image--wrapper {if $variant.variant_id == $feature.variant_id}ty-product-options__image--wrapper--active{/if} {if $quick_view}cm-ajax cm-ajax-cache{/if}"
                                   {if $quick_view}data-ca-target-id="{$container}"{/if}
                                >
